...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/connect/youtube?success=true`))
  } catch (err) {
    console.error('Error in YouTube callback:', err)
    return NextResponse.redirect(new URL(`${process.env.NEXT_PUBLIC_SITE_URL}/connect/youtube?error=unknown`))
  }
}