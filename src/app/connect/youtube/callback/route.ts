...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(new URL(`/connect/youtube?success=true`, request.url))
  } catch (err) {
    console.error('Error in YouTube callback:', err)
    return NextResponse.redirect(new URL(`/connect/youtube?error=unknown`, request.url))
  }
}